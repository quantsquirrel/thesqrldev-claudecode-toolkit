...
# ---------------------------------------------------------------------------
CITE_RE = re.compile(r"\S+\.(?:py|ts|js|go|rs|md|json|toml):\d+", re.IGNORECASE)
...
def _rule_uncited_numeric(line_text: str, line_no: int, full_text: str, line_start: int):
    ...
    """
    pattern = re.compile(r"~?\d[\d,]*\s*(?:줄|lines?|LoC|tests?|tests?개|sessions?)", re.IGNORECASE)
    warnings = []
    for m in pattern.finditer(line_text):
        abs_end = line_start + m.end()
        if not _is_cited(full_text, abs_end):
...
def _rule_existence_claim(line_text: str, line_no: int, full_text: str, line_start: int):
    ...
    """
    pattern = re.compile(r"(?:providers|modules|lib|src)/\S*", re.IGNORECASE)
...
def _rule_test_count_claim(line_text: str, line_no: int, full_text: str, line_start: int):
    ...
    """
    pattern = re.compile(r"\d+/\d+\s*(?:regression|tests?|green)", re.IGNORECASE)
...
def main():
    parser = argparse.ArgumentParser(description="Audit a prompt string for unbacked claims.")

...
> Current default wiring: Gemini → `agy-cli` (Antigravity, Gemini 3.5 Flash family),
> OpenAI → `cliproxy-cli` (CLIProxyAPI localhost:8317).
...
GEMINI_MODEL="${GEMINI_MODEL:-3.5-flash}"
GEMINI_THINKING="${GEMINI_THINKING:-medium}"
OPENAI_MODEL="${OPENAI_MODEL:-gpt55fast}"
...
if [[ -n "$PROBLEM_TYPE" && "$PROBLEM_TYPE" != "general" ]]; then
    case "$PROBLEM_TYPE" in
        coding)
            ...
            ;;
        math)
            # CLIProxyAPI defaults already encode xhigh reasoning in gpt55fast.
            if [[ "$OPENAI_MODEL" == "gpt54mini" ]]; then
                echo "[ProblemType] math → OpenAI: gpt54mini → gpt55fast" >&2
                OPENAI_MODEL="gpt55fast"
            fi
            ;;
        creative)
            # agy-cli is intentionally pinned to Gemini 3.5 Flash; keep model family stable.
            ;;
    esac
fi
```
...
|------|--------------|-----------------|--------------|------------------|-------------|---------|
| `review` | 3.5-flash | high | gpt55fast | - | 3 | Yes (2-4) |
| `design` | 3.5-flash | high | gpt55fast | - | 4 | Yes (3-4) |
| `debug` | 3.5-flash | high | gpt55fast | - | 3 | Yes (2-4) |
| `idea` | 3.5-flash | high | gpt55fast | - | 4 | Yes (3-4) |
| `general` | 3.5-flash | medium | gpt55fast | - | 3 | Yes (2-4) |
...
|-------|-----------------|------|-------|
| Gemini 3.5 Flash via agy-cli (Solver/Defense) | high | `--thinking high` accepted/ignored by wrapper | 창의성 + 정확성 균형 |
| Gemini 3.5 Flash via agy-cli (Critic) | medium | `--thinking medium` accepted/ignored by wrapper | 분석적 평가 |
| CLIProxyAPI gpt55fast | xhigh | model alias encodes effort | priority tier + 심층 추론 |
| CLIProxyAPI gpt54mini | default | model alias encodes effort | 빠른 응답 |
...
**대체 제어**: CLIProxyAPI는 `--reasoning` 값을 호환성용으로 받지만 실제 강도는 모델 alias(`gpt55fast`, `gpt55`, `gpt55high`)에 포함됩니다.

### CLIProxyAPI Model by Mode

| Mode | model alias | 설명 |
|------|------------------|------|
| review | gpt55fast | 균형 잡힌 분석 |
| design | gpt55fast | 심층 아키텍처 추론 |
| debug | gpt55fast | 근본 원인 분석 |
| idea | gpt55fast | 창의적 탐색 |
| general | gpt55fast | 기본 응답 |
...
```json
{
  ...
  "model_config": {
    "gemini": {"model": "{3.5-flash}", "thinking": "{medium|high}"},
    "openai": {"model": "{gpt55fast|gpt55|gpt54mini}", "reasoning": "{null}"}
  },
  "total_rounds": {3|4}
}
```
